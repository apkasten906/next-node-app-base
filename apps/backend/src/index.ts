...
export class App {
  ...
  private initializeRoutes(): void {
    ...
    // Redirect to Swagger UI so visiting http://localhost:3001/ is useful.
    this.app.get('/', (_req: Request, res: Response) => {
      res.setHeader('cache-control', 'no-store');
      res.redirect(302, '/api-docs');
    });

    // Serve test reports locally (development only).
    // NOTE: These files are generated by running Cucumber (e.g. `pnpm -C apps/backend test:bdd`).
