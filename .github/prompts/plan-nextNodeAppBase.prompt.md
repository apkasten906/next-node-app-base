...
1. **BDD base-repo scenario coverage** — see section N for full scope breakdown. Observability, file-storage, notifications, message-queue, and advanced-testing are now complete for base scenarios. Priority order within BDD work: kubernetes-devops → frontend base scenarios. Message-queue is now complete for the base slice. Websocket, security, observability, file-storage, notifications, message-queue, and advanced-testing base slices are complete. Target: all ~162 base-repo scenarios promoted to `@ready`.
