...
Every scenario must have exactly one primary status tag:

- `@ready` — enabled by default (runs in CI/local default profile)
- `@wip` — not ready yet (kept for requirements capture and iteration)
- `@manual` — intentionally manual (documented but not automated)

Optional:

- `@skip` — temporarily disabled (e.g. flaky/blocked)
